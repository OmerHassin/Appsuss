...
function googleQuery(bookName) {
  console.log(bookName, 'book name');
  return fetch(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${bookName}`)
    .then((response) => response.json())
    .then((data) => {
      const books = data.items.map((item) => {
        const { id, volumeInfo, saleInfo } = item;
        const { pageCount, publishedDate, title, imageLinks } = volumeInfo;
        const { saleability } = saleInfo;
        const thumbnail = imageLinks ? imageLinks.thumbnail : null;
        return { id, pageCount, publishedDate, title, saleability, thumbnail };
      });
...
function query(filterBy = {}) {
  return storageService.query(BOOK_KEY).then((books) => {
    if (filterBy.title) {
      ...
      console.log(filterBy.title, books);
    }
...
function addGoogleBook(book) {
  storageService.query(BOOK_KEY).then((books) => {
...
function addReview(bookId, review) {
  storageService.get(BOOK_KEY, bookId).then((currentBook) => {
